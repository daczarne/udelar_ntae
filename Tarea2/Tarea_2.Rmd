...
```{r, fig.cap="Scatter plot of life expectancy at birth (in year) and years with lm regression lines, by continent"}
gapminder %>% 
   ggplot(aes(x = lifeExp, y = year, color = continent)) + 
   geom_point() +
   geom_smooth(method = "lm", se = FALSE) +
   labs(x = "Life expectancy at birth (in years)", y = "Time (in years)", color = NULL) +
...
```{r}
ggplot(gapminder) +
   geom_line(aes(x = lubridate::make_date(year), 
                 y = gdpPercap, color = continent, group = country)) +
   facet_wrap(~continent) +
   labs(x = "Time (in years)", y = "GDP Per capita (USD, inflation-adjusted)", color = NULL) +
   theme(axis.title = element_text(face = "bold"))
```
...
ggplot(mpg, aes(x = drv)) +
   geom_bar(aes(y = ..prop.., fill = factor(..x..), group = "mira_que_lindos_grupos"),
            stat="count", show.legend = FALSE) +
   geom_text(aes(label = scales::percent(..prop..), y = ..prop..-0.075, 
                 group = "mira_que_lindos_grupos"), 
